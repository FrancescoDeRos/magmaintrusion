...
!~ ---------------------------------------------------------------------
      ...
      Subroutine SOLVE_STA_CRACK(Ut,Ub,x0z0_in,delta_in,Hd_in,DPvisc,A_iter,n_dis_in,esc)
...
!~       Subroutine COMP_DYN_DPvisc_OLD(Ut_old,Ut,Hd_,n_dis,DPvisc)
!~ !compute the viscous pressure change within a moving crack 
!~ !Ut_old(:) INPUT
!~ !Ut(:)     INPUT
!~ !Hd_(:)    INPUT
...
!~       INSUM(1) = (Ut(1) - Ut_old(1)) - Ut_old(1)*(A-A_old)/A_old               ! - Ut(1)*(A-A_old)/A
!~       Do j=2, n_dis-1
!~         INSUM(j) = INSUM(j-1) + (Ut(j) - Ut_old(j)) - Ut_old(j)*(A-A_old)/A_old! - Ut(j)*(A-A_old)/A
!~       EndDo
!~       INSUM(n_dis) = (INSUM(n_dis-1) + Ut(n_dis))                              ! - Ut(n_dis)*(A-A_old)/A
!~ !***********************************************************************************

!~ !***********************************************************************************
!~ !**************************** INCOMPRESSIBLE FLUID *********************************
!~ !***********************************************************************************
!      INSUM(1) = (Ut(1) - Ut_old(1))
!      Do j=2, n_dis-1
!        INSUM(j) = (INSUM(j-1) + (Ut(j) - Ut_old(j)))
!      EndDo
!      INSUM(n_dis) = (INSUM(n_dis-1) + Ut(n_dis))
!~ !***********************************************************************************

!~ !***** EQ-1 ************************************************************************
!      DPvisc(1) = ( (12.D0*v*eta*Hd_(1)) / ((Ut(1)*0.5D0+Ut_old(1)*0.5D0)**3) ) * INSUM(1)
!      Do i=2, n_dis-1
!        DPvisc(i) = DPvisc(i-1) + ( (12.D0*v*eta*Hd_(i)) / ((Ut(i)*0.5D0+Ut_old(i)*0.5D0)**3) ) * INSUM(i)
!      EndDo
!      DPvisc(n_dis) = DPvisc(n_dis-1) + ( (12.D0*v*eta*Hd_(n_dis)) / ((Ut(n_dis)*0.5D0)**3) ) * INSUM(n_dis)
!~ !***********************************************************************************

!~ !***** EQ-2 ************************************************************************
!~       DPvisc(1) = ( (12.D0*v*eta*Hd_(1)) / (Ut_old(1))**3 ) * INSUM(1)
!~       Do i=2, n_dis-1
!~         DPvisc(i) = DPvisc(i-1) + ( (12.D0*v*eta*Hd_(i)) / (Ut_old(i))**3 ) * INSUM(i)
!~       EndDo
!~       DPvisc(n_dis) = DPvisc(n_dis-1)
!~ !***********************************************************************************
...
!~       End Subroutine COMP_DYN_DPvisc_OLD
...
10    B(1:n_dis)   = -( sig(1:n_dis) + Kappa + Pidr*DeZ(1:n_dis)*(A0/A_iter) ) ! MPa
      ...
      If ( (abs((A_iter_old-A_iter)/A_iter_old).gt.1.D-10).and.(counter.le.10) ) GoTo 10

      If (counter.gt.10) Then
        write (*,*) '** WARNING ** counter>10 in sub COMP_UtUb'
!~         read (*,*)
      EndIf
...
10    B(1:n_dis)   = -(sig(1:n_dis) + Kappa + Pidr*DeZ(1:n_dis)*(A0/A_iter) + DPvisc(1:n_dis)) ! MPa
      ...
      If (counter.gt.10) write (*,*) '** WARNING ** counter>10 in sub COMP_DYN_UtUb'
...
!~ ---------------------------------------------------------------------
      ...
      Subroutine COMP_OverP (Ut,Ub,x0z0,delta,Hd_,DPvisc,A,n_dis,OverP,OverP_av)
...
!~ OverP_av OUTPUT
      ...
      REAL(8) :: Ut(:),Ub(:),x0z0(:,:),delta(:),Hd_(:),DPvisc(:),OverP(:)
      REAL(8) :: A,OverP_av
      INTEGER :: n_dis
...
!~ ---------------------------------------------------------------------
!~ ---------------------------------------------------------------------
!      Subroutine COMP_D_OLD (Ut,Ut_old,x0z0,x0z0_old,Hd_,D)
!!~ compute viscous dissipation energy of the system
!!~ Ub     INPUT
!!~ Ub_old INPUT
!!~ Hd_    INPUT
!!~ D      OUTPUT
...
!      REAL(8) :: Ut(:),Ut_old(:),Hd_(:),x0z0(:,:),x0z0_old(:,:)
!      REAL(8),ALLOCATABLE :: h(:),h_old(:), xz(:,:),SUM_Ut(:)
!      REAL(8) :: D,MULT
!      INTEGER :: n, n_utold, ndiff
!      INTEGER :: i,j
      
!      D = 0.D0
!      n = 1
!      n_utold = size(Ut_old)  
!      ndiff = size(Ut_old)-size(Ut)
      
!      MULT = -12.D0*v*eta*1.D6 ! keep the opening in [mm] or [m] and multiply by 1.D6 this coefficient to adjust the dimention, numerically more efficient.

!      IF(x0z0_old(1,1) .NE. 0.D0) then
!	    DO WHILE ((x0z0(1,1) .NE. x0z0_old(n,1)) .OR. (x0z0(1,2) .NE. x0z0_old(n,2)))
!	      n=n+1
!	    ENDDO
!	    n=n-1
	    
!	    IF (ndiff .ge. 0) then
!	      ALLOCATE(h(n_utold+n),h_old(n_utold+n),xz(n_utold+n,2),SUM_Ut(n_utold+n))
!	    ELSEIF (ndiff .lt. 0) then
!	      ALLOCATE(h(n_utold-ndiff),h_old(n_utold-ndiff),xz(n_utold-ndiff,2),SUM_Ut(n_utold-ndiff))
!	    ENDIF
	    
!        h(:) = 0.D0
!        h_old(:) = 0.D0
        
!        call SORT_vec(Ut,Ut_old,h,h_old,x0z0,x0z0_old,xz,n_utold,n,ndiff) 
        
!!~         write(*,*)n_utold, size(Ut)
!!~         do i=1,n_utold+1
!!~           write(*,*)h_old(i),h(i)
!!~         enddo
!!~         read(*,*)        
...
!        SUM_Ut(:)=0.D0
!        SUM_Ut(1)=h(1)-h_old(1)
!        DO i=2,n_utold+1
!          SUM_Ut(i)=SUM_Ut(i-1)+(h(i)-h_old(i))
!        ENDDO  

!        Do i = 1,n_utold+1
!          if (h_old(i) .GT. 0.D0) then
!            write(*,*)h_old(i),SUM_Ut(i),(h_old(i))**3,(SUM_Ut(i))**2
!            D = D + ( (MULT*(Hd_(i))**2) / (h_old(i))**3 ) * (SUM_Ut(i))**2
!            !D=D+((12*viscosity*velocity*Hd_(i)*1e3*Hd_(i)*1e3)/(h_old(i)*h_old(i)*h_old(i)))*(SUM_Ut)*(SUM_Ut) ![mm], *viscosity*velocity [kPa.m.mm]
!          else
!            D=D+0
!          Endif
!        EndDo
...
!	    DEALLOCATE(h,h_old,SUM_Ut)

!	  ENDIF

      

!      End Subroutine COMP_D_OLD
...
!~ ---------------------------------------------------------------------
      ...
      Subroutine update_v(DeE,Diss,dJ,vel)
...
!~ ---------------------------------------------------------------------
      ...
      Subroutine set_vel(vel,new_vel)
...
!~ ---------------------------------------------------------------------
!~ ---------------------------------------------------------------------
!      Subroutine SORT_vec(Ut,Ut_old,h,h_old,x0z0,x0z0_old,xz,n_utold,n,ndiff)
!!~ compute viscous dissipation energy of the system
!!~ Ut         INPUT
!!~ Ut_old     INPUT
!!~ x0z0       INPUT
...
!      REAL(8) :: Ut(:),Ut_old(:),x0z0(:,:),x0z0_old(:,:)
!      REAL(8) :: h(:),h_old(:),xz(:,:)
!      INTEGER :: n, n_utold, ndiff
!      INTEGER :: i,j
      
  
!!~       write(111,*)n_utold, n, size(Ut), ndiff, size(h)
!!~       write(111,*)'LOOP 1'
!      do i=1,n
!        xz(i,1)=x0z0_old(i,1)
!        xz(i,2)=x0z0_old(i,2)
...
!      enddo
!      if (ndiff .lt. 0) then
!!~         write(111,*)'LOOP 3 case ndiff .lt. 0'
!        do i=n_utold+1,n_utold-ndiff
!          xz(i,1)=x0z0(j,1)
!          xz(i,2)=x0z0(j,2)
!          h(i)=Ut(j)
!          h_old(i)=0.D0
...
!!~         write(111,*)'LOOP 3 case ndiff .eq. 0'
!        do i=n_utold+1,n_utold+n
!          xz(i,1)=x0z0(j,1)
!          xz(i,2)=x0z0(j,2)
!          h(i)=Ut(j)
!          h_old(i)=0.D0
...
!!~           write(111,*)i,xz(i,:),h(i),h_old(i)
!        enddo
!      elseif (ndiff .gt. 0) then
!!~         write(111,*)'LOOP 3 case ndiff .gt. 0'
!        do i=n_utold+1,n_utold+ndiff
!          xz(i,1)=x0z0(j,1)
!          xz(i,2)=x0z0(j,2)
!          h(i)=Ut(j)
