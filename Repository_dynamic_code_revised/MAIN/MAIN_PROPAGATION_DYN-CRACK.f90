      ...
      Use PROPAGATION,    Only : SET_PROP_PAR, START_PROP, SET_ANALOG_PATH
      ...
      INTEGER :: n_dir
      REAL(8) :: alfa
      REAL(8) :: x_stop_min,x_stop_max,z_stop,z_load
      REAL(8) :: E_prop
      
      INTEGER :: i,run,n_dikes,dike_run, nb_ana
...
! SET ROCK PARAMETERS
      ...
      Read(41,*)                     ! row 4
      Read(41,*) z_dns, rhoR2, free_surf, z_ana	! row 5 - z_dns [km] depth of density transition 
												!       - rhoR2 [kg/dm^3] rock density in the upper layer (z<0)
												!       - free_surf is boolean, if .false. there is no free surface in z=0
												!       - z_ana is boolean, if .true. the path is imposed
...
! SET THE MAGMA PARAMETERS
      ...
      Read(41,*)                 ! row 6
      Read(41,*) rhoM, Kappa!, A0 ! row 7 [kg/dm^3] magma density / [MPa] magma Bulk modulus / reference volume of magma (cross section)
      Read(41,*)                 ! row 8
...
!-------------------------------------------------------------------!
      ...
      Read(41,*)                                          ! row 25
      Read(41,*) x_stop_min, x_stop_max, z_stop, z_load   ! row 26 boundaries for the dyke propagation paths
      Read(41,*)                                          ! row 27
      Read(41,*) E_prop                                   ! row 28 energy threashold for propagation

      alfa = alfa*pi/180.D0

!-------------------------------------------------------------------!
!-------------------------------------------------------------------!
! SET IMPOSED PATH IF Z_ANA is TRUE
	  If (z_ana.eqv..true.) then
!~ !!!!!!!!!!!!!!!!!!!IMPOSED PATH!!!!!!!!!!!!!!!!!!!
		inpf02 = './input/z0delta_analog.txt'
		Open(42,file = inpf02,access='sequential',form='formatted')
		Read(42,*) nb_ana
		Close(42)
		call SET_ANALOG_PATH(nb_ana)
!~ !!!!!!!!!!!!!!!!!!!IMPOSED PATH!!!!!!!!!!!!!!!!!!!
	  EndIf
...
! SET THE PARAMETERS NEEDED FOR THE DIKE PROPAGATION
      ...
      Call SET_PROP_PAR(n_iter,n_dir,alfa,x_stop_min,x_stop_max,z_stop,z_load, &
     &                  Dz_snap,E_prop,E_prop,E_prop,vel_var,z_ana)
!-------------------------------------------------------------------!
